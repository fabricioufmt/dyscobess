...
std::string DyscoOut::GetDesc() const {
  return bess::utils::Format("%s/%s", port_->name().c_str(),
                             port_->port_builder()->class_name().c_str());
}

void DyscoOut::ProcessBatch(bess::PacketBatch *batch) {
  Port *p = port_;
...
int DyscoOut::OnEvent(bess::Event e) {
...
ADD_MODULE(DyscoOut, "dysco_out", "sends pakets to a port (Dysco).")
