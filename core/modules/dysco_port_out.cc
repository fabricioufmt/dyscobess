...
CommandResponse DyscoPortOut::Init(const bess::pb::DyscoPortOutArg &arg) {
...
std::string DyscoPortOut::GetDesc() const {
  return bess::utils::Format("%s/%s", port_->name().c_str(),
                             port_->port_builder()->class_name().c_str());
}

void DyscoPortOut::ProcessBatch(bess::PacketBatch *batch) {
  Port *p = port_;
...
int DyscoPortOut::OnEvent(bess::Event e) {
...
ADD_MODULE(DyscoPortOut, "dysco_port_out", "sends pakets to a port")
