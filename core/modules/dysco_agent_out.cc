 ...
 * The packet is either LockingSignal or not.
 * Control packets are processed by DyscoAgentIn.
 */
...
void DyscoAgentOut::out_translate(Packet*, Ipv4* ip, Tcp* tcp, DyscoHashOut* cb_out) {
	...
	if(!other_path) {
		fprintf(stderr, "[%s] is not other_path with state=%d\n", ns.c_str(), cb_out->state);
		if(isTCPACK(tcp))
		...
		if(other_path->state == DYSCO_ESTABLISHED) {
			...
			if(seg_sz > 0) {
				fprintf(stderr, "[%s] I'm going to compare after(seq, cb_out->seq_cutoff)\n", ns.c_str());
				fprintf(stderr, "after(%X, %X) == %u\n", seq, cb_out->seq_cutoff, after(seq, cb_out->seq_cutoff));
