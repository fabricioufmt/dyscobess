...
CommandResponse DyscoInc::Init(const bess::pb::DyscoIncArg &arg) {
...
std::string DyscoInc::GetDesc() const {
	return bess::utils::Format("%s/%s", port_->name().c_str(),
				   port_->port_builder()->class_name().c_str());
}

struct task_result DyscoInc::RunTask(void *arg) {
...
CommandResponse DyscoInc::CommandSetBurst(
...
ADD_MODULE(DyscoInc, "dysco_inc", "receives packets from a port (Dysco).")
