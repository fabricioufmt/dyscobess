...
#ifdef DEBUG_RECONFIG
	fprintf(stderr, "[%s][DyscoAgentIn-Control] cb_in->seq_delta == 0\n", ns.c_str());
#endif		
	return false;
}
...
#ifdef DEBUG_RECONFIG
	fprintf(stderr, "[%s][DyscoAgentIn-Control] cb_in->ack_delta == 0\n", ns.c_str());
#endif		
	return false;
}
...
bool DyscoAgentIn::compute_deltas_in(DyscoHashIn* cb_in, DyscoHashOut* old_out, DyscoCbReconfig* rcb) {
	...
	if(cb_in->in_iseq < cb_in->out_iseq) {
		...
		cb_in->seq_delta = ntohl(cb_in->in_iseq - cb_in->out_iseq);
#ifdef DEBUG_RECONFIG
	...
	if(cb_in->in_iack < cb_in->out_iack) {
		...
		cb_in->ack_delta = ntohl(cb_in->in_iack - cb_in->out_iack);
		cb_in->ack_add = 0;
	}
...
bool DyscoAgentIn::compute_deltas_out(DyscoHashOut* cb_out, DyscoHashOut* old_out, DyscoCbReconfig* rcb) {
	...
	if(cb_out->in_iack < cb_out->out_iack) {
		cb_out->ack_delta = ntohl(cb_out->out_iack - cb_out->in_iack);
		cb_out->ack_add = 1;
	} else {
		cb_out->ack_delta = ntohl(cb_out->in_iack - cb_out->out_iack);
		cb_out->ack_add = 0;
	}
