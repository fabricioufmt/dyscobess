...
const Commands DyscoPortInc::cmds = {
    {"set_burst", "DyscoPortIncCommandSetBurstArg",
     MODULE_CMD_FUNC(&DyscoPortInc::CommandSetBurst), Command::THREAD_SAFE},
};
...
std::string DyscoPortInc::GetDesc() const {
  return bess::utils::Format("%s/%s", port_->name().c_str(),
                             port_->port_builder()->class_name().c_str());
}

struct task_result DyscoPortInc::RunTask(void *arg) {
...
CommandResponse DyscoPortInc::CommandSetBurst(
...
ADD_MODULE(DyscoPortInc, "dysco_port_inc", "receives packets from a port")
