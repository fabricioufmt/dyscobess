...
void create_segment(struct iphdr*, struct tcphdr*, uint16_t, uint16_t, struct tcpopt*, uint8_t, uint8_t, uint32_t, uint32_t*);
...
int main(int argc, char** argv) {
	...
	create_segment(iph, tcph, htons(atoi(argv[2])), htons(atoi(argv[4])), tcpo, atoi(argv[5]), atoi(argv[6]), sc_len, sc);
...
void create_segment(struct iphdr* iph, struct tcphdr* tcph, uint16_t sport, uint16_t dport, struct tcpopt* tcpo, uint8_t lhop, uint8_t rhop, uint32_t sc_len, uint32_t* sc) {
	...
	psh.tcp_length = htons((tcph->doff << 2) + sc_len * sizeof(uint32_t));
	uint32_t size = sizeof(struct pseudo_header) + (tcph->doff << 2) + sc_len * sizeof(uint32_t);
	uint8_t* buff = malloc(size);

	memcpy(buff, &psh, sizeof(struct pseudo_header));
	memcpy(buff + sizeof(struct pseudo_header), tcph, tcph->doff << 2);
	memcpy(buff + sizeof(struct pseudo_header) + (tcph->doff << 2), sc, sc_len * sizeof(uint32_t));
